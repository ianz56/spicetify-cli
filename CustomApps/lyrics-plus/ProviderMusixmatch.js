const ProviderMusixmatch = (() => {
    ...
    const baseURL = `https://apic-desktop.musixmatch.com/ws/1.1/crowd.track.translations.get?translation_fields_set=minimal&comment_format=text&format=json&app_id=web-desktop-app-v1.0&`;

    const languagesToTry = ["en", "id"];
    let result = null;
    ...
    for (const language of languagesToTry) {
      ...
      result = await CosmosAsync.get(finalURL, null, headers);

      if (
        result.message.header.status_code === 200 &&
        result.message.body.translations_list?.length
      ) {
        // If translations are found, break the loop
        break;
      }
    }
    ...
    return result.message.body.translations_list.map(({ translation }) => ({
